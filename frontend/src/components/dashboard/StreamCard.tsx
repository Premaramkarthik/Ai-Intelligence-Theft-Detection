...
import { Camera, Maximize2, Activity, ShieldCheck, AlertCircle } from 'lucide-react';
import { CameraSocket } from '@/lib/socket';
...
export default function StreamCard({ id, name }: StreamCardProps) {
    ...
    useEffect(() => {
        const socket = new CameraSocket(`ws://localhost:9001/ws/camera/${id}`, {
            onMessage: (data) => {
                // 1. Update Video Frame (Base64)
                if (data.frame && videoRef.current) {
                    videoRef.current.src = data.frame;
                    if (!isLive) {
                        console.log(`Stream ${id} received first frame`);
                        setIsLive(true);
                    }
                }
                ...
                if (canvasRef.current) {
                    ...
                    if (data.action && data.action.label !== 'normal') {
                        ctx.fillStyle = 'rgba(239, 68, 68, 0.8)'; // Red semi-transparent
                        ctx.fillRect(0, 0, canvasRef.current.width, 40);
                        ctx.fillStyle = '#FFFFFF';
                        ctx.font = 'bold 20px Inter';
                        ctx.fillText(`TRIGGER: ${data.action.label.toUpperCase()} (${Math.round(data.action.confidence * 100)}%)`, 20, 28);
                    }
                    ...
                    data.detections.forEach((det: any) => {
                        ...
                        const color = isPerson ? '#3B82F6' : '#F59E0B'; // Blue for person, Amber for item
    ...
    return (
        <div className="group relative bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-xl transition-all hover:border-blue-500/30">
            {/* Header */}
